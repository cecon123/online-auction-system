...
public class RequestRouter {
  ...
  public Response<?> route(Request<?> request) {
    ...
    try {
      return switch (type) {
        case LOGIN -> {
          com.auction.common.dto.auth.LoginResponse authRes =
              authService.login(requireData(request, LoginRequest.class, "Missing login data"));
        ...
        case LOGOUT -> {
          ...
          yield Response.ok(type, request.getRequestId(), "Logged out successfully", null);
        }
        case PLACE_BID -> {
          Long userId = requireActiveUser(request);
          PlaceBidRequest bidData = requireData(request, PlaceBidRequest.class, "Missing bid data");
          yield Response.ok(
              type, request.getRequestId(), "Bid accepted", bidService.placeBid(userId, bidData));
        }
        ...
        case GET_BID_HISTORY -> {
          ...
          if (auctionId == null) {
            yield Response.fail(type, request.getRequestId(), "Missing auction ID");
          }
          yield Response.ok(
              type,
              request.getRequestId(),
        ...
        case SUBSCRIBE_AUCTION -> {
          ...
          yield Response.ok(
              type, request.getRequestId(), "Subscribed to auction " + auctionId, null);
        }
        case UNSUBSCRIBE_AUCTION -> {
          ...
          yield Response.ok(
              type, request.getRequestId(), "Unsubscribed from auction " + auctionId, null);
        }
  ...
  private Response<DashboardDto> handleGetDashboard(Request<?> request) {
    ...
    if (user.role() == Role.BIDDER) {
      ...
      activeAuctionsCount =
          (int) sellerAuctions.stream().filter(a -> a.getStatus() == AuctionStatus.RUNNING).count();
    } else if (user.role() == Role.ADMIN) {
      List<Auction> allAuctions = auctionDao.findAll();
      totalAuctionsCount = allAuctions.size();
      activeAuctionsCount =
          (int) allAuctions.stream().filter(a -> a.getStatus() == AuctionStatus.RUNNING).count();
  ...
  private Response<Void> handleSetAutoBid(Request<?> request) {
    ...
    com.auction.common.dto.bid.SetAutoBidRequest data =
        requireData(
            request,
  ...
  private Response<List<AuctionSummaryDto>> handleGetAuctions(Request<?> request) {
    List<Auction> auctionList = auctionDao.findAll();
    return mapToAuctionSummaries(auctionList, request.getRequestId());
  }

  private Response<List<AuctionSummaryDto>> handleGetSellerAuctions(Request<?> request) {
    Long userId = requireActiveUser(request);
  ...
  private Response<com.auction.common.dto.dashboard.SellerStatsDto> handleGetSellerStats(
    ...
    com.auction.common.dto.dashboard.SellerStatsDto stats =
        new com.auction.common.dto.dashboard.SellerStatsDto(
            expectedRevenue,
            totalRevenue,
            totalBidsReceived,
            successRate,
            ...
            .map(
                a -> {
                  ...
                  BigDecimal startingPrice =
                      (item != null) ? item.getStartingPrice() : a.getCurrentPrice();
  ...
  private Response<AuctionDetailDto> handleGetAuctionDetail(Request<?> request) {
    Long auctionId = jsonMapper.convertData(request.getData(), Long.class);
    if (auctionId == null) {
      throw new IllegalArgumentException("Missing auction ID");
    }
    ...
    AuctionDetailDto detail =
        new AuctionDetailDto(
            ...
            item.getDescription(),
            auction
                .getCurrentPrice(), // startingPrice (should ideally be from auction too if tracked)
  ...
  private Response<BigDecimal> handleDeposit(Request<?> request) {
    ...
    BigDecimal amount = jsonMapper.convertData(request.getData(), BigDecimal.class);
    if (amount == null) {
  ...
  private Response<AuctionSummaryDto> handleCreateAuction(Request<?> request) {
    Long userId = requireActiveUser(request);

    CreateAuctionRequest data =
        requireData(request, CreateAuctionRequest.class, "CREATE_AUCTION requires auction payload");

    validateCreateAuctionRequest(data);

    // Process image upload
    String savedFileName =
        com.auction.server.util.ImageUtil.saveBase64Image(
            data.imageBase64(),
  ...
  private Response<?> handleUpdateAuction(Request<?> request) {
    ...
    com.auction.common.dto.auction.UpdateAuctionRequest data =
        requireData(
            request,
    ...
    if (savedFileName != null) {
      finalRequest =
          new com.auction.common.dto.auction.UpdateAuctionRequest(
              data.auctionId(),
              data.itemName(),
              data.itemType(),
              data.condition(),
  ...
  private void validateUpdateAuctionRequest(
      com.auction.common.dto.auction.UpdateAuctionRequest data) {
    if (data.auctionId() <= 0) {
  ...
  private Response<List<com.auction.common.dto.bid.BidHistoryDto>> handleGetUserBidHistory(
      Request<?> request) {
    Long userId = requireActiveUser(request);
    ...
    List<com.auction.common.dto.bid.BidHistoryDto> enrichedHistory =
        history.stream()
            .map(
                h -> {
                  String title =
                      auctionDao
                          .findById(h.auctionId())
                          .flatMap(a -> itemDao.findById(a.getItemId()))
                          .map(Item::getName)
                          .orElse(h.auctionTitle());

                  return new com.auction.common.dto.bid.BidHistoryDto(
                      h.bidId(), h.auctionId(), title, h.amount(), h.timestamp(), h.result());
                })
            .toList();
  ...
  private <T> T requireData(Request<?> request, Class<T> clazz, String errorMessage) {
