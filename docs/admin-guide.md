...
- **Tài khoản demo khác:** `seller01`, `seller02`, `bidder01`, `bidder02` đều dùng password `password123`.
...
- **Đã kết thúc (Finished):** Số lượng các cuộc đấu giá không còn hoạt động, bao gồm `FINISHED`, `PAID` và `CANCELED`.
...
- **Huy hiệu trạng thái:** `OPEN`, `RUNNING`, `FINISHED`, `PAID`, `CANCELED`.
- **Các hành động quản trị:**
...
2. Đăng nhập vào **Client A** với tài khoản `admin` / `password123`.
3. Đăng nhập vào **Client B** với tài khoản `seller01` / `password123`.
...
7. Quan sát **Client B**: Trạng thái đấu giá sẽ cập nhật thành `CANCELED`.
