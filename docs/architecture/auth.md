...
2. **Server:** 
    ...
    - Lưu token vào `SessionManager` kèm theo `userId`.
...
2. Kiểm tra tính hợp lệ của token trong `SessionManager` để lấy `userId`.
...
- **Session:** Token phiên làm việc chỉ tồn tại trong bộ nhớ RAM của Server và sẽ bị xóa khi người dùng đăng xuất (`LOGOUT`) hoặc khi Server khởi động lại.
