...
public class AdminClientService {
  ...
  public CompletableFuture<Response<List<UserDto>>> getUsers() {
    ...
    return socketClient
        .<Void, List<UserDto>>sendRequest(request)
        .thenApply(
            response -> {
  ...
  public CompletableFuture<Response<Void>> updateUserStatus(Long userId, boolean active) {
    ...
    Request<UpdateUserStatusRequest> request =
        new Request<>(MessageType.ADMIN_UPDATE_USER_STATUS, null, null, data);
  ...
  public CompletableFuture<Response<List<AuctionDetailDto>>> getAuctions() {
    ...
    return socketClient
        .<Void, List<AuctionDetailDto>>sendRequest(request)
        .thenApply(
            response -> {
              if (response.isSuccess()) {
                List<AuctionDetailDto> data =
                    jsonMapper.convertList(response.getData(), AuctionDetailDto.class);
                response.setData(data);
              }
              return response;
            });
