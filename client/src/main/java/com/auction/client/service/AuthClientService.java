...
public class AuthClientService {
  ...
  public CompletableFuture<Response<LoginResponse>> login(String username, String password) {
    ...
    return socketClient
        .<LoginRequest, LoginResponse>sendRequest(request)
        .thenApply(
            response -> {
              if (response.isSuccess()) {
                LoginResponse data =
  ...
  public CompletableFuture<Response<RegisterResponse>> register(RegisterRequest registerData) {
    Request<RegisterRequest> request =
        new Request<>(MessageType.REGISTER, null, null, registerData);

    return socketClient
        .<RegisterRequest, RegisterResponse>sendRequest(request)
        .thenApply(
            response -> {
              if (response.isSuccess()) {
                RegisterResponse data =
                    jsonMapper.convertData(response.getData(), RegisterResponse.class);
                response.setData(data);
