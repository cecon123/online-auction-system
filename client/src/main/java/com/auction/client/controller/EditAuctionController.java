...
public class EditAuctionController {
    ...
    private void handleSaveAuction() {
        ...
        if (startDate == null) { showError("Start date is required."); return; }
        if (endDate == null) { showError("End date is required."); return; }

        LocalDateTime startTime = LocalDateTime.of(startDate,
            LocalTime.of(startHourSpinner.getValue(), startMinuteSpinner.getValue()));
        LocalDateTime endTime = LocalDateTime.of(endDate,
