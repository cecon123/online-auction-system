        ...
        .addEventListener(
            ...
            response -> {
              try {
                ...
                if (event != null) {
                  boolean isWinner =
                      event.winnerUsername() != null
                          && event
                              .winnerUsername()
                              .equals(com.auction.client.util.SceneManager.getCurrentUsername());

                  String title = "Auction Ended";
                  String message =
                      String.format(
                          "Auction #%d has ended. Status: %s", event.auctionId(), event.status());
                  ...
                  if (event.status() == com.auction.common.enums.AuctionStatus.FINISHED) {
                    if (isWinner) {
                      ...
                      message =
                          String.format(
                              "You won Auction #%d with a bid of $%,.2f!",
                              event.auctionId(), event.finalPrice().doubleValue());
                    } else {
                      // Check if current user is seller (we don't have sellerId in event, but we
                      // can infer from dashboard refresh later)
                      // For now, general sold message
                      title = "Item Sold!";
                      message =
                          String.format(
                              "Auction #%d finished at $%,.2f.",
                              event.auctionId(), event.finalPrice().doubleValue());
                    }
                  }
                  ...
                  // Trigger a dashboard refresh to update balance/stats for everyone (Buyer gets
                  // deducted, Seller gets paid)
            ...
            response -> {
              try {
                ...
                if (event != null) {
                  ...
                  // Refresh balance/stats for any system notification (often involves financial
                  // changes)
                  new com.auction.client.service.AuctionClientService()
                      .getDashboard()
                      .thenAccept(
