...
public class AdminPanelController {
  ...
  @FXML private VBox auctionTableContainer;
                ...
                Platform.runLater(
                    () -> {
                      ...
                      runningAuctionsLabel.setText(
                          String.format("%,d", stats.activeAuctionsCount()));
                      finishedAuctionsLabel.setText(
                          String.format(
                              "%,d", stats.totalAuctionsCount() - stats.activeAuctionsCount()));
                    });
              }
            });
  }
                ...
                Platform.runLater(
                    () -> {
                      ...
                      if (userTableContainer.getChildren().size() > 1) {
                        userTableContainer
                            .getChildren()
                            .remove(1, userTableContainer.getChildren().size());
                      }

                      for (int i = 0; i < users.size(); i++) {
                        userTableContainer
                            .getChildren()
                            .add(createUserRow(users.get(i), i % 2 != 0));
                      }
                    });
              } else {
                Platform.runLater(
                    () -> messageLabel.setText("Failed to load users: " + response.getMessage()));
              }
            });
  }
                    ...
                    () -> {
                      ...
                      if (auctionTableContainer.getChildren().size() > 1) {
                        auctionTableContainer
                            .getChildren()
                            .remove(1, auctionTableContainer.getChildren().size());
                      }

                      for (int i = 0; i < auctions.size(); i++) {
                        auctionTableContainer
                            .getChildren()
                            .add(createAuctionRow(auctions.get(i), i % 2 != 0));
                      }
                    });
              } else {
                Platform.runLater(
                    () ->
                        messageLabel.setText("Failed to load auctions: " + response.getMessage()));
              }
            });
  }
  ...
  private GridPane createUserRow(UserDto user, boolean alt) {
    ...
    Label statusLabel =
        createStatusBadge(statusText, user.active() ? "status-active" : "status-suspended");
    ...
    actionBtn
        .getStyleClass()
        .addAll("admin-pill-btn", user.active() ? "admin-btn-danger" : "admin-btn-success");
  ...
  private GridPane createAuctionRow(AuctionDetailDto auction, boolean alt) {
    ...
    String endTimeFormatted =
        auction.endTime() != null ? auction.endTime().format(DATE_FORMATTER) : "N/A";
                          ...
                          Platform.runLater(
                              () -> {
                                if (res.isSuccess()) {
                                  ...
                                  messageLabel.setText(
                                      "Failed to cancel auction: " + res.getMessage());
                                }
                              });
                        });
              }
