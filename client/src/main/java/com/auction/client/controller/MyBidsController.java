...
public class MyBidsController {
  ...
  @FXML private VBox participatingAuctionsContainer;
  @FXML private VBox bidHistoryContainer;
  @FXML private Label activeBidsLabel;
  @FXML private Label winningBidsLabel;
  @FXML private Label outbidLabel;
  ...
  private final Consumer<Response<?>> refreshListener =
      response -> {
  ...
  private void populateParticipatingAuctions(List<AuctionSummaryDto> auctions) {
    ...
    if (participatingAuctionsContainer.getChildren().size() > 1) {
      participatingAuctionsContainer
          .getChildren()
          .remove(1, participatingAuctionsContainer.getChildren().size());
    }
    ...
    for (int i = 0; i < auctions.size(); i++) {
      ...
      boolean userIsLeading =
          auction.highestBidderId() != null && auction.highestBidderId() == currentUserId;
  ...
  private GridPane createAuctionRow(
      AuctionSummaryDto auction, boolean isAlt, boolean userIsLeading) {
    GridPane row = new GridPane();
    row.setHgap(20);
