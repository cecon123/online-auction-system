                  ...
                  () -> {
                    if (response.isSuccess()) {
                      BigDecimal newBalance =
                          jsonMapper.convertData(response.getData(), BigDecimal.class);
                      balance = newBalance;
                      SceneManager.setCurrentBalance(balance);
                      refreshBalance();
                      messageLabel.setText(
                          "Deposit successful! New balance: " + CURRENCY.format(balance));
                      amountField.clear();
                    } else {
                      messageLabel.setText("Deposit failed: " + response.getMessage());
                    }
                  });
            });
  }
              ...
              Platform.runLater(
                  () -> {
                    if (response.isSuccess()) {
                      ...
                      refreshBalance();
                      messageLabel.setText(
                          "Withdraw successful! New balance: " + CURRENCY.format(balance));
                      amountField.clear();
                    } else {
                      messageLabel.setText("Withdraw failed: " + response.getMessage());
                    }
                  });
            });
  }
